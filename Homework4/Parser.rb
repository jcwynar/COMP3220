...
class Parser < Scanner
    ...
    def exp()
        ...
        if (@lookahead.type == Token::ADDOP or @lookahead.type == Token::SUBOP)
            exp = etail()
            exp.addAsFirstChild(start_term)
            exp.shiftSiblingsDown()
        else
            exp = start_term
        end
    ...
    def term()
        ...
        if (@lookahead.type == Token::MULTOP or @lookahead.type == Token::DIVOP)
            term = ttail()
            term.addAsFirstChild(start_factor)
            term.shiftSiblingsDown()
        else
            term = start_factor
        end
        return term
    ...
    def ttail()
        ...
        if (@lookahead.type == Token::MULTOP)
            ...
            rec_ttail = ttail()
            if (rec_ttail != nil)
                rec_ttail.addAsFirstChild(ttail)
                ttail = rec_ttail
            end
    ...
    def etail()
        ...
        if (@lookahead.type == Token::ADDOP)
            ...
            rec_etail = etail()
            if (rec_etail != nil)
                rec_etail.addAsFirstChild(etail)
                etail = rec_etail
            end
